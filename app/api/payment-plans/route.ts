...
export async function POST(request: NextRequest) {
  ...
  // Super Admin only: payment plan creation
  try {
    requireSuperAdmin(auth);
  } catch (e) {
    return NextResponse.json({error: 'Forbidden: Only Super Admin can create payment plans'}, {status: 403});
  }
  ...
  try {
    ...
    await logAudit({ tenantId: auth.tenantId, userId: auth.userId, action: 'create', resourceType: 'payment_plan', resourceId: result[0].id, before: null, after: result[0] });
